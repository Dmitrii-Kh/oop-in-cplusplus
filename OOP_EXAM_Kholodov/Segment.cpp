...
const Segment& Segment::rotateSegment(int angle)
{
	_a = start().rotatePoint(angle);
	_b = end().rotatePoint(angle);
	return *this;
}

const Segment& Segment::mirrorSegment(char axis)
{
	_a = start().mirrorPoint(axis);
	_b = end().mirrorPoint(axis);
	return *this;
